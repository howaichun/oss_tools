...
$log_path = __DIR__."/Logs/okaycms/".date('Ymd',time()).".update.static.log";
$origin_log_path = __DIR__."/Logs/okaycmsOrigin/".date('Ymd',time()).".update.static.log";
$err_path = __DIR__."/Error/okaycms/".date('Ymd',time()).".error.log";

/**
 * @Desc : 遍历对应数据库的需要操作的数据表
 */
foreach($cms_okay as $tableName=>$val){
	// echo count($val)."\n";continue;
	$sql = "select ";   
    	...
    	foreach($rowVal as $k=>$v){
            ...
            if($n!=0){
                 if(!empty($v)){
                    ...
                    //普通的URL处理
                    $locateFullPath =  getCmsImageRealPath($v);
                    ...
                    if($fileExistYesOrNot){
                         ...
                         //OSS处理上传图片 
                         try{
                             $result = storeToAliyunOss($OSSClient, $locateFullPath, $v,'okaycms');  
                            // if(!empty($result)){
                                $updateSql .= " `".$k."`='".$result['url_short_store']. "' ,";  
                           //  }
                         } catch (Exception $e) {
                            $e->getErrorMessage();
                         }
